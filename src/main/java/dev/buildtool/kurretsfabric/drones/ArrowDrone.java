...
public class ArrowDrone extends Drone {
    public DefaultInventory weapon = new DefaultInventory(1) {
        @Override
    ...
    public ArrowDrone(EntityType<? extends MobEntity> entityType, World world) {
        ...
        if (target.isAlive()) {
            ...
            if (!weapon.isEmpty()) {
                for (ItemStack item : ammo.getItems()) {
                    if (!item.isEmpty()) {
                        PersistentProjectileEntity projectile = ProjectileUtil.createArrowProjectile(this, item, pullProgress);
